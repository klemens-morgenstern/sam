...
def _b2_command(source_dir, toolset, cxxstd, variant, stdlib='native', address_model='64'):
...
def _linux_pipeline(name, image, command):
    return {
        "name": name,
        "kind": "pipeline",
        "type": "docker",
...
def linux_b2(
    ...
    return _linux_pipeline(name, image, command)
