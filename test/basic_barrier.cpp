...
struct basic_barrier_main : net::coroutine
{
  ...
  basic_barrier_main(net::any_io_executor exec) : impl_(std::make_unique<basic_barrier_main_impl>(exec)) {}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(cancel, T, models)
{
  ...
  asio::post([&] { sig.emit(asio::cancellation_type::total); });
...
BOOST_AUTO_TEST_CASE(mt_shutdown)
{
  ...
  std::exception_ptr     ep;
  {
    ...
    thr = std::thread([smtx] { BOOST_CHECK_THROW(smtx->arrive(), system_error); });
