...
TEST_SUITE_BEGIN("basic_shared_mutex_test::unique");
...
TEST_CASE("rebind_shared_mutex" * doctest::timeout(10.))
{
  net::io_context ctx;
  auto            res = net::deferred.as_default_on(shared_mutex{ctx.get_executor()});
}
...
TEST_CASE_TEMPLATE("cancel_lock" * doctest::timeout(10.), T, io_context, thread_pool)
{
  ...
  {
    shared_mutex::template rebind_executor<net::io_context::executor_type>::other mtx{ctx};
    mtx.async_lock(res);
    mtx.async_lock(res);
    mtx.async_lock(res);
    mtx.async_lock(res);
...
TEST_CASE_TEMPLATE("shutdown_" * doctest::timeout(10.), T, io_context, thread_pool)
{
  ...
  auto       smtx = std::make_shared<shared_mutex>(ctx);
...
TEST_CASE("holds-work" * doctest::timeout(10.))
{
  ...
  CHECK(ctx.run_for(std::chrono::milliseconds(100)) == 0);
}
...
TEST_SUITE_BEGIN("basic_shared_mutex_test::shared");
...
TEST_CASE("sync_lock_st" * doctest::timeout(10.))
{
  ...
  CHECK_NOTHROW(mtx.lock_shared());
  CHECK(mtx.try_lock_shared());
  CHECK_NOTHROW(mtx.lock_shared());
}
...
TEST_CASE("sync_lock_mt" * doctest::timeout(10.))
{
  ...
  mtx.lock_shared();
  std::thread thr{[&]{ mtx.lock_shared(); }};
  mtx.lock_shared();
  thr.join();
}


TEST_CASE_TEMPLATE("cancel_shared" * doctest::timeout(10.), T, io_context, thread_pool)
{
  net::io_context        ctx{init<T>()};
  std::vector<error_code> ecs;
  auto                    res = [&](error_code ec) { ecs.push_back(ec); };
...
TEST_CASE_TEMPLATE("lock_shared" * doctest::timeout(10.), T, io_context, thread_pool)
{
  net::io_context        ctx{init<T>()};
  std::vector<error_code> ecs;
  auto                    res = [&](error_code ec) { ecs.push_back(ec); };
  ...
  CHECK(ecs.size() == 7u);
  CHECK(0u == std::count(ecs.begin(), ecs.end(), error::operation_aborted));
}

TEST_CASE_TEMPLATE("unlock_shared" * doctest::timeout(10.), T, io_context, thread_pool)
{
  net::io_context        ctx{init<T>()};
  std::vector<error_code> ecs;
  auto                    res = [&](error_code ec) { ecs.push_back(ec); };
...
TEST_SUITE_BEGIN("basic_shared_mutex_test");
...
TEST_CASE("sync_lock_st" * doctest::timeout(10.))
{
  ...
  CHECK_THROWS(mtx.lock());
  mtx.unlock_shared();
  CHECK_NOTHROW(mtx.lock());
  CHECK_THROWS(mtx.lock_shared());
  CHECK(!mtx.try_lock_shared());
...
TEST_CASE("sync_lock_mt_1" * doctest::timeout(10.))
{
  net::io_context ctx;
  shared_mutex            mtx{ctx};
...
TEST_CASE("sync_lock_mt_1" * doctest::timeout(10.))
{
  ...
  mtx.lock_shared();
  net::post(ctx, [&] { mtx.unlock_shared(); });
  std::atomic<bool> locked{false};
